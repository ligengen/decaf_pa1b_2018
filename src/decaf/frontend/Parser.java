...
public class Parser extends Table {
    ...
    private SemValue parse(int symbol, Set<Integer> follow) {
        ...
        Set<Integer> end_set = followSet(symbol);
        Set<Integer> begin_set = beginSet(symbol);
        end_set.addAll(follow);

        if(result == null) {
            error();
            flag = true;
            do {
                if(begin_set.contains(lookahead)) {
                    return parse(symbol, follow);
                }
                else if(end_set.contains(lookahead)) {
                    return null;
                }
                else {
                    lookahead = lex();
        ...
        for (int i = 0; i < length; i++) { // parse right-hand side symbols one by one
            ...
            params[i + 1] = isNonTerminal(term)
                    ? parse(term, end_set) // for non terminals: recursively parse it
